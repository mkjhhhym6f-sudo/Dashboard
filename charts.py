...
x=norm_p.index, y=norm_p.values, name=ticker,
line=dict(color=UDES_GOLD, width=2.5),
))
fig.add_trace(go.Scatter(
x=norm_b.index, y=norm_b.values, name=“Benchmark”,
...
x=valid_labels, y=values, marker=dict(color=colors),
text=[”{:+.1f}%”.format(v) for v in values],
...
text = [
[(”{:” + fmt + “}”).format(v) if pd.notna(v) else “” for v in row]
...
text=”<b>{}</b><br><span style='font-size:10px'>SECTORS</span>”.format(len(sectors)),
...
fill=“tozeroy”, fillcolor=color + “1A”, name=title,
))
if threshold is not None:
fig.add_hline(
y=threshold, line_dash=“dot”, line_color=NEUTRAL,
...
line=dict(width=1, color=TEXT_PRIMARY), opacity=0.85,
),
...
return _apply_theme(go.Figure(), “No data”, 380)
text = [
[”${:.0f}”.format(v) if v else “-” for v in row]
for row in df.values
]
fig = go.Figure(go.Heatmap(
z=df.values,
...
textfont=dict(size=11, color=TEXT_PRIMARY),
colorbar=dict(
title=”$/share”, thickness=10, len=0.8,
tickfont=dict(color=TEXT_MUTED, size=10),
),
))
if current_price is not None:
fig.add_annotation(
xref=“paper”, yref=“paper”, x=1.15, y=1.05,
