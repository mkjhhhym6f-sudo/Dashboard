...
def load_universe() -> pd.DataFrame:
    """
    Load universe.csv and normalise to a schema every page can rely on.

    Guaranteed output columns regardless of CSV schema:
        ticker      — primary key used by all pages
        ciq_symbol  — alias for Capital IQ lookups
        ticker_yf   — Yahoo Finance symbol
        name        — display name
        sector, industry, currency, is_etf, analyst

    Supports both old schema (ticker, ticker_yf, name) and
    new schema (ciq_symbol, yahoo_symbol, company) transparently.
    """
    ...
    try:
        ...
        # ── Step 1: normalise primary symbol column → 'ticker' ───────────────
        # Old schema: has 'ticker' already
        # New schema: has 'ciq_symbol', no 'ticker'
        if "ticker" not in df.columns:
            if "ciq_symbol" in df.columns:
                df["ticker"] = df["ciq_symbol"]
            elif "symbol" in df.columns:
                df["ticker"] = df["symbol"]
            else:
                # No symbol column at all — can't proceed
                logger.error("universe.csv has no ticker/ciq_symbol/symbol column")
                return pd.DataFrame()

        # ── Step 2: ensure ciq_symbol always exists ───────────────────────────
        if "ciq_symbol" not in df.columns:
            df["ciq_symbol"] = df["ticker"]

        # ── Step 3: normalise Yahoo Finance symbol → 'ticker_yf' ─────────────
        if "ticker_yf" not in df.columns:
            if "yahoo_symbol" in df.columns:
                df["ticker_yf"] = df["yahoo_symbol"]
            else:
                # Derive from ticker using the same logic as get_yf_ticker()
                def _to_yf(t: str) -> str:
                    t = str(t)
                    if t in SPECIAL_TICKER_MAP:
                        return SPECIAL_TICKER_MAP[t]
                    if ":" in t:
                        prefix, base = t.split(":", 1)
                        if prefix == "TSX":
                            return f"{base}.TO"
                        return base
                    return t
                df["ticker_yf"] = df["ticker"].apply(_to_yf)

        # ── Step 4: normalise display name → 'name' ──────────────────────────
        ...
        # ── Step 5: ensure 'company' alias also exists ────────────────────────
        if "company" not in df.columns:
            df["company"] = df["name"]

        # ── Step 6: fill in optional columns with safe defaults ───────────────
        if "sector" not in df.columns:
            df["sector"] = "Unknown"
        if "industry" not in df.columns:
            # Try subsector (old schema) — safe column access, no index mismatch
            df["industry"] = df["subsector"] if "subsector" in df.columns else ""
        ...
        # ── Step 7: normalise is_etf to bool ──────────────────────────────────
        if "is_etf" in df.columns:
            df["is_etf"] = df["is_etf"].astype(str).str.upper().isin(["TRUE", "1", "YES"])
        else:
            df["is_etf"] = False

        # ── Step 8: strip example / placeholder rows ──────────────────────────
        mask_example = (
            df["ticker"].astype(str).str.startswith("EXAMPLE") |
            df["ticker"].astype(str).str.startswith("IQ_SYMBOL") |
            df["name"].astype(str).str.contains("EXAMPLE", case=False, na=False)
        )
        df = df[~mask_example].copy()

        # ── Step 9: strip whitespace from all string columns ──────────────────
        for col in ["ticker", "ciq_symbol", "ticker_yf", "name", "company",
                    "sector", "industry", "currency", "analyst"]:
