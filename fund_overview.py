...
def render():
    ...
    for _, row in snap.iterrows():
        ...
        result = compute_composite_score(fund, rets, row.get("sector", "Default"),
    ...
    kpis = [
        ...
        ("1-Day", fmt_pct(weighted_avg("ret_1d"), signed=True), color_for_value(weighted_avg("ret_1d"))),
        ("1-Month", fmt_pct(weighted_avg("ret_1m"), signed=True), color_for_value(weighted_avg("ret_1m"))),
        ("YTD", fmt_pct(weighted_avg("ret_ytd"), signed=True), color_for_value(weighted_avg("ret_ytd"))),
        ("1-Year", fmt_pct(weighted_avg("ret_1y"), signed=True), color_for_value(weighted_avg("ret_1y"))),
    ]
    ...
    with c2:
        ...
        fig = go.Figure(go.Bar(x=ytds, y=sector_grp_sorted["sector"],
                                orientation="h", marker=dict(color=colors),
                                text=[f"{v:+.1f}%" for v in ytds],
                                textposition="outside"))
        fig.update_xaxes(title_text="YTD Return (%)")
        fig.update_layout(showlegend=False)
        st.plotly_chart(_apply_theme(fig, "Average Sector YTD Return", height=380),
    ...
    st.plotly_chart(heatmap(hm_pct, "Return Heatmap (%) — All Positions",
                             zmin=-30, zmax=30, fmt="+.1f"),
                     use_container_width=True)
    ...
    def render_ranking(df_in: pd.DataFrame):
        ...
        df_show["score"] = df_show["score"].apply(lambda v: f"{v:.0f}" if is_valid(v) else "N/A")
        df_show.columns = ["Ticker", "Company", "Sector", "Price",
                            "1M", "YTD", "1Y", "EV/EBITDA", "Score", "Rec"]
    ...
    if not alerts:
        st.markdown('<div class="alert-box alert-success">✅ No critical alerts — portfolio appears healthy across all positions</div>',
                     unsafe_allow_html=True)
    else:
        c1, c2 = st.columns(2)
    ...
    st.download_button("📥 Export Portfolio Snapshot to CSV", data=csv_data,
                        file_name="portfolio_snapshot.csv", mime="text/csv")
